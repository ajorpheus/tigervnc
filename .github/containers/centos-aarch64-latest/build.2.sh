...
docker run --volume ${CURDIR}/rpmbuild:/home/rpm/rpmbuild --interactive --rm tigervnc/${DOCKER} \
	bash -e -x -c "
	sudo dnf install -yqqq xorg-x11-server-devel;
	sudo dnf install -yqqq https://kojihub.stream.centos.org/kojifiles/packages/xorg-x11-font-utils/7.5/53.el9/x86_64/xorg-x11-font-utils-7.5-53.el9.x86_64.rpm;
	sudo dnf builddep -yqqq ~/rpmbuild/SPECS/tigervnc.spec;
